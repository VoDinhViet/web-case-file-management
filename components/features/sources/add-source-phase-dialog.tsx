...
import { addPhaseToSource } from "@/actions/source-phase";
...
interface AddSourcePhaseDialogProps {
  sourceId: string;
}

export function AddSourcePhaseDialog({ sourceId }: AddSourcePhaseDialogProps) {
  ...
  function onSubmit(values: CreatePhaseDto) {
    startTransition(async () => {
      try {
        const result = await addPhaseToSource({
          sourceId,
          rawData: values,
        });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto scrollbar-thin">
        <DialogHeader>
          <DialogTitle>Thêm giai đoạn mới cho nguồn tin</DialogTitle>
          <DialogDescription>
