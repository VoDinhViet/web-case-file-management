...
import { formatFieldValue } from "@/components/features/cases/case-detail/format-utils";
...
interface SourceCustomGroupsProps {
  sourceData: Source;
}

export function SourceCustomGroups({ sourceData }: SourceCustomGroupsProps) {
  const hasGroups = sourceData.groups && sourceData.groups.length > 0;
  const hasCustomFields =
    sourceData.customFields && Object.keys(sourceData.customFields).length > 0;
  ...
  return (
    <>
      {hasGroups &&
        sourceData.groups?.map((group, index) => {
          const colors = [
            {
          ...
          return (
            ...
            >
              <CardHeader
                className={`pb-4 bg-gradient-to-br ${colorScheme.bg} to-white dark:to-background border-b pt-4 !pb-2`}
              >
      ...
      {hasCustomFields && (
        <Card className="border-pink-200 dark:border-pink-900 overflow-hidden">
          ...
          <CardContent>
            <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
              {Object.entries(sourceData.customFields || {}).map(
                ([key, value]) => (
                  <div
                    key={key}
                    className="group p-3 rounded-lg border border-transparent hover:border-primary/20 hover:bg-accent/50 transition-colors"
