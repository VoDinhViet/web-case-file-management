...
import {
  deleteSourcePhase,
  toggleSourcePhaseCompletion,
} from "@/actions/source-phase";
import { EditSourcePhaseDialog } from "@/components/features/sources/edit-source-phase-dialog";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
interface SourcePhaseItemProps {
  phase: SourcePhase;
  sourceId: string;
}

function SourcePhaseItem({ phase, sourceId }: SourcePhaseItemProps) {
  const {
    id,
    name,
    description,
  ...
  const handleToggle = useCallback(() => {
    startTransition(async () => {
      const result = await toggleSourcePhaseCompletion({
        phaseId: id,
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const result = await deleteSourcePhase({
        sourceId,
        phaseId: id,
      });
          ...
          <CardHeader>
            <div className="flex items-start justify-between gap-4">
              ...
              <CardAction className="flex items-center gap-1">
                ...
                <EditSourcePhaseDialog sourceId={sourceId} phase={phase} />
                <Button
                  size="icon"
                  variant="ghost"
...
interface SourcePhasesTimelineProps {
  sourceId: string;
  phases: SourcePhase[];
}

export function SourcePhasesTimeline({
  sourceId,
  phases,
}: SourcePhasesTimelineProps) {
  if (phases.length === 0) {
    return (
      <Card className="border-dashed">
        <CardContent className="pt-6">
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-xs">
              Thêm giai đoạn để theo dõi tiến độ nguồn tin
            </p>
          </div>
        </CardContent>
      </Card>
    ...
    <div className="relative space-y-8">
      ...
      {sortedPhases.map((phase, index) => (
        <div key={phase.id} className="relative">
          <SourcePhaseItem phase={phase} sourceId={sourceId} />
